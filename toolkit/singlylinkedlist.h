...
public:
	...
	// Adds data to the back, i.e a new tail.
	void add(T data);
	...
	void addByIndex(T data, unsigned int index);
	...
	// Creates a singly linked list starting with the given data. 
	...
	// Removes all instances of the specified data. NYI.
	...
	// Performs a linear search of the data to find the input data and returns the index it is found at. Returns -1 if it cannot be found. Useful for unsorted lists. NYI.
	bool find(T data, unsigned int &index);
